...
import 'package:sizzle_starter/src/core/api_client/exception.dart';
import 'package:sizzle_starter/src/core/utils/meta.dart';

/// {@template api_client}
/// A client for making HTTP requests.
/// {@endtemplate}
abstract interface class ApiClient {
  /// Sends `GET` request to the given [path].
  ...
  /// Sends `POST` request to the given [path].
  ...
  /// Sends `PUT` request to the given [path].
  ...
  /// Sends `PATCH` request to the given [path].
  ...
  /// Sends `DELETE` request to the given [path].
...
/// {@template api_client_base}
/// A base class for implementing [ApiClient].
/// {@endtemplate}
abstract base class ApiClientBase implements ApiClient {
  /// {@macro api_client_base}
  const ApiClientBase({required this.baseUri});
  ...
  @protected
  @visibleForTesting
  @Throws({ApiClientInternalException})
  List<int> encodeBody(Map<String, Object?> body) {
    try {
      return _jsonUTF8.encode(body);
    } on Object catch (e, stackTrace) {
      Error.throwWithStackTrace(
        ApiClientInternalException('Error occurred during encoding', error: e),
        stackTrace,
      );
    }
  }
  ...
  /// [ApiClientStructuredException] with the error details.
  ///
  ...
  Future<Map<String, Object?>?> decodeResponse(
    ...
    try {
      ...
      if (decodedBody case {'error': final Map<String, Object?> error}) {
        throw ApiClientStructuredException(error, statusCode: statusCode);
      }
